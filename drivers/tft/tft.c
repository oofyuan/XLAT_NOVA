...
static void sdram_verify_and_retry(void);
...
void tft_init(void)
{
    ...
    /* Some boards have a marginal SDRAM that can fail to initialize at cold
     * boot, making the LTDC read garbage (large vertical stripes). Verify the
     * framebuffer region and retry the SDRAM init a few times. */
    sdram_verify_and_retry();
...
static void sdram_verify_and_retry(void)
{
    const uint32_t words = TFT_HOR_RES * TFT_VER_RES;
    const uint32_t stride = 997; /* prime: exercises the whole area */
    ...
    for (int attempt = 0; attempt < 5; attempt++) {
        ...
        for (i = 0; i < words; i += stride) {
            my_fb[i] = (uint16_t)(0xA55A ^ (uint16_t)(i * 3));
        }
        for (i = 0; i < words; i += stride) {
            if (my_fb[i] != (uint16_t)(0xA55A ^ (uint16_t)(i * 3))) {
                ok = 0;
                break;
            }
        }
    ...
    /* Clear the framebuffer again after any re-initialization */
