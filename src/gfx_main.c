...
{
    lv_label_set_text_fmt(latency_label,
                          gfx_i18n_tr("#%lu: %ldus, avg %ldus, stdev %ldus",
                                     "#%lu: %ldus, 平均 %ldus, 标准差 %ldus"),
...
static void utf8_copy_chars(char *dst, size_t dst_size, const char *src, size_t max_chars)
{
    ...
    while (*p && chars < max_chars && dst_pos + 1 < dst_size) {
        unsigned char c = *p;
        size_t len = 1;
        ...
        if (dst_pos + len >= dst_size) {
            break;
        }

        for (size_t i = 0; i < len; i++) {
            dst[dst_pos++] = (char)p[i];
        }
        p += len;
        chars++;
    }
...
void gfx_device_label_set(const char * manufacturer, const char * productname, const char *vidpid)
{
    ...
    utf8_copy_chars(tempstr, sizeof(tempstr), productname, 20);
    ...
    utf8_copy_chars(tempstr2, sizeof(tempstr2), manufacturer, 20);
    lv_label_set_text(manufacturer_label, tempstr2);
...
static void auto_trigger_clear_timer(void)
{
    ...
    lv_label_set_text(trigger_label, gfx_i18n_tr("TRIGGER", "触发"));
...
void gfx_data_locations_label_set(void)
{
    ...
    if (*button_bits || *motion_bits) {
        sprintf(text, gfx_i18n_tr("Data(%u): %u buttons, %u motion bits",
                                  "数据(%u): %u个按键, %u个移动位"),
                xlat_report_id_get(), *button_bits, *motion_bits);
    } else if (keyboard_usage_page_found) {
        sprintf(text, gfx_i18n_tr("Data(%u): keyboard detected",
                                  "数据(%u): 检测到键盘"),
                xlat_report_id_get());
    } else {
        // offsets not found
        sprintf(text, "%s", gfx_i18n_tr("Data: no data bits found",
                                        "数据: 未找到数据位"));
    }
...
void gfx_mode_label_set(void)
{
    ...
    switch (xlat_mode_get()) {
        case XLAT_MODE_MOUSE_CLICK:
            mode_str = gfx_i18n_tr("Click", "点击");
            break;
        case XLAT_MODE_MOUSE_MOTION:
            mode_str = gfx_i18n_tr("Motion", "移动");
            break;
        case XLAT_MODE_KEYBOARD:
            mode_str = gfx_i18n_tr("Key", "按键");
            break;
        default:
            mode_str = gfx_i18n_tr("Unknown", "未知");
    }

    sprintf(text, gfx_i18n_tr("Mode: %s -", "模式: %s -"), mode_str);
...
void gfx_xlat_gui(void)
{
    ...
    gfx_device_label_set("", gfx_i18n_tr("No USB device detected", "未检测到USB设备"), "");
    ...
    lv_label_set_text(clear_label, gfx_i18n_tr("Clear", "清除"));
    ...
    // Reboot button label
    reboot_label = lv_label_create(reboot_btn);
    lv_label_set_text(reboot_label, gfx_i18n_tr("Reboot", "重启"));
    ...
    lv_label_set_text(settings_label, gfx_i18n_tr("Settings", "设置"));
    ...
    lv_label_set_text(latency_label, gfx_i18n_tr("Click to start measurement...",
                                                 "点击开始测量..."));
    lv_obj_align_to(latency_label, chart, LV_ALIGN_OUT_TOP_MID, 0, 0);

    // Trigger ready label
    trigger_ready_cb = lv_checkbox_create(lv_scr_act());
    lv_checkbox_set_text(trigger_ready_cb, gfx_i18n_tr("Ready", "就绪"));
...
void gfx_main_language_changed(void)
{
    if (device_connected) {
        gfx_labels_update();
    } else {
        gfx_device_label_set("", gfx_i18n_tr("No USB device detected",
                                             "未检测到USB设备"), "");
    }
